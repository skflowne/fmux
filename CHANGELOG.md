...
- **The titlebar now exposes host and WSL resource pressure before workflows fall over.** A compact `CPU · RAM · WSL` readout updates every five seconds; hovering shows exact host RAM, wmux RSS, and WSL RAM/swap usage. Healthy values stay neutral, 75–84% turns warning amber, and 85%+ turns red so exhausted WSL swap cannot remain invisible until every terminal exits.
